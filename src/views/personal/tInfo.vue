<template>
  <div>
    <el-descriptions :column="2" size="medium" title="我的信息" direction="horizontal" border style="width: 94%;margin-left: 3%;margin-top: 70px;margin-bottom: 20px">
      ...
      <el-descriptions-item span="2" label="关联教师信息">
        <el-descriptions v-if="showTeacher" :column="4" direction="vertical" border>
          <el-descriptions-item label="教工号">{{ teacherData.no }}</el-descriptions-item>
          <el-descriptions-item label="姓名">{{ teacherData.name }}</el-descriptions-item>
          <el-descriptions-item label="学院">{{ teacherData.college }}</el-descriptions-item>
          <el-descriptions-item label="职位">{{ teacherData.position }}</el-descriptions-item>
          <el-descriptions-item label="操作"><el-button type="danger" @click="unbindTeacher">解绑教师</el-button></el-descriptions-item>
        </el-descriptions>
        <el-popover
          placement="right"
          width="298"
          trigger="click">
          <el-form :inline="true" :model="teacherNo" class="demo-form-inline">
            <el-form-item>
              <el-input v-model="teacherNo.teacherNo" placeholder="请输入教工号" @keydown.enter.native="bindTeacher"/>
            </el-form-item>
            <el-form-item>
              <el-button type="primary" style="margin-left: -14px" @click="bindTeacher">绑定</el-button>
            </el-form-item>
          </el-form>
          <el-link v-if="!showTeacher" slot="reference" :underline="false" type="primary">绑定教师信息</el-link>
        </el-popover>
      </el-descriptions-item>
...
export default {
  ...
  methods: {
    ...
    // 绑定教师
    bindTeacher() {
      TeacherApi.bindTeacher(this.userId, this.teacherNo.teacherNo).then(response => {
        if (response.code === 200) {
          this.$notify({
            title: '成功',
            message: '绑定教师成功！',
    ...
    unbindTeacher() {
      TeacherApi.unbindTeacher(this.userId).then(response => {
        if (response.code === 200) {
