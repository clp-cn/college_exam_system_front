<template>
  <div>
    <el-descriptions :column="2" size="medium" title="我的信息" direction="horizontal" border style="width: 94%;margin-left: 3%;margin-top: 70px;margin-bottom: 20px">
      ...
      <el-descriptions-item span="2" label="关联学生信息">
        <el-descriptions v-if="showStudent" :column="3" direction="vertical" border>
          <el-descriptions-item label="学号">{{ studentData.no }}</el-descriptions-item>
          ...
          <el-descriptions-item label="专业">{{ studentData.profession }}</el-descriptions-item>
          <el-descriptions-item label="年级">{{ studentData.grade }}</el-descriptions-item>
          <el-descriptions-item label="班级">{{ studentData.className }}</el-descriptions-item>
          <el-descriptions-item label="操作"><el-button type="danger" @click="unbindStudent">解除绑定</el-button></el-descriptions-item>
        </el-descriptions>
        <el-popover
          placement="right"
          width="298"
          trigger="click">
          <el-form :inline="true" :model="studentNo" class="demo-form-inline">
            <el-form-item>
              <el-input v-model="studentNo.studentNo" placeholder="请输入学号" auto-complete="on" @keydown.enter.native="bindStudent"/>
            </el-form-item>
            <el-form-item>
              <el-button type="primary" style="margin-left: -14px" @click="bindStudent">绑定</el-button>
            </el-form-item>
          </el-form>
          <el-link v-if="!showStudent" slot="reference" :underline="false" type="primary">绑定学生信息</el-link>
        </el-popover>
      </el-descriptions-item>
      ...
      <el-descriptions-item span="2" label="操作">
        ...
        <el-button v-if="!showExam" type="success" style="margin-left: 30px" @click="showExamCard">展示准考证</el-button>
        <el-button v-if="showExam" type="info" style="margin-left: 30px" @click="hiddenExamCard">隐藏准考证</el-button>
      </el-descriptions-item>
    </el-descriptions>
    <!--    修改信息-->
    <el-dialog
    ...
    <el-descriptions v-if="showExam" :column="2" size="medium" title="准考证信息" direction="horizontal" border style="width: 50%;margin-left: 26%;margin-top: 20px;margin-bottom: 20px">
      <el-descriptions-item span="2" label="考号">{{ studentData.no }}</el-descriptions-item>
      <el-descriptions-item span="2" label="姓名">{{ studentData.name }}</el-descriptions-item>
      <el-descriptions-item span="2" label="性别">{{ studentData.sex }}</el-descriptions-item>
      <el-descriptions-item span="2" label="班级">{{ studentData.className }}</el-descriptions-item>
    </el-descriptions>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    // 绑定学生
    bindStudent() {
      StudentApi.bindStudent(this.userId, this.studentNo.studentNo).then(response => {
        if (response.code === 200) {
          this.$notify({
            title: '成功',
            message: '绑定学生成功！',
    ...
    unbindStudent() {
      StudentApi.unbindStudent(this.userId).then(response => {
        if (response.code === 200) {
          this.$notify({
            title: '成功',
            message: '解绑学生成功！',
